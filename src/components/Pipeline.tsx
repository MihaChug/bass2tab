...
const STAGES: Stage[] = [
  ...
  {
    num: "02",
    title: "Предобработка (CPU)",
    engine: "torchaudio.functional",
    desc: "Миксдаун в моно, ресемплинг до 16 kHz (Kaiser, β≈14.8), biquad-highpass 30 Гц против сценического гула и нормализация пика до −1 dBFS. Считается на CPU — это секунды, зато первое выделение GPU-памяти происходит уже под защитой дочернего процесса (на macOS 27.0 Metal умеет abort'ить крупные аллокации).",
  ...
  },
  {
    ...
    tags: ["model=full", "viterbi", "чанки 30 c", "360 bins × 20¢"],
    icon: ICONS.chip,
  },
  {
    num: "04",
