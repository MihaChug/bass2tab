...
const FAQ = [
  {
    ...
    a: "Это падает не Python, а Metal-драйвер macOS: CREPE выделяет больше общей GPU-памяти, чем драйвер готов отдать, и тот завершает процесс SIGABRT — перехватить его try/except принципиально нельзя. Поэтому в pitch.py инференс на MPS теперь запускается в ДОЧЕРНЕМ процессе: если драйвер его убивает (exitcode != 0), родитель ловит это и автоматически повторяет весь прогон на CPU — транскрибация завершится в любом случае. Дополнительно трек идёт чанками по 30 с с очисткой Metal-кэша, а cli.py до импорта torch выставляет PYTORCH_ENABLE_MPS_FALLBACK=1. Обновите bass2tabs/pitch.py и cli.py из «Исходников». Если хотите остаться на GPU и снизить шанс аборта, уменьшите батч: --batch 128 или --batch 64; помогает и закрытие GPU-прожорливых приложений. Гарантированный обход — --device cpu (точность та же, медленнее).",
  },
  {
