...
Постобработка высоты (порядок важен):
  * декодирование — ВСЕГДА Витерби (не argmax): алгоритм динамического
    программирования связывает соседние фреймы в единые ноты, отсекает
    скачущий шум и минимизирует «октавные ошибки»;
  * сглаживание частоты — torchcrepe.filter.mean (окно 3 кадра), устойчиво
    к NaN: убирает микро-скачки и квантовочные артефакты;
  * обнуление уверенности в абсолютной тишине — невзвешенный RMS (−60 дБ),
    чистый numpy. Намеренно НЕ torchcrepe.threshold.Silence: его A-взвешивание
    глушит басовые фундаменты на 22–43 дБ, и весь бас выглядел бы «тишиной».
...
def _crepe_pass(audio, sr, hop, model, device, batch) -> tuple[np.ndarray, np.ndarray]:
    ...
    for ci in range(n_chunks):
        ...
        # Декодер — ВСЕГДА Витерби (не argmax): алгоритм динамического
        # программирования лучше связывает соседние фреймы в единые ноты и
        # отсекает случайный скачущий шум, минимизируя «октавные ошибки»
        # (суб-бас не прыгает на октаву вверх). Витерби — декодер по умолчанию
        # в torchcrepe, фиксируем его явно.
        ...
        # Сглаживание частоты (убирает микро-скачки и квантовочные артефакты):
        # скользящее среднее по окну 3 кадра (~30 мс при hop 10 мс).
        # torchcrepe.filter.mean устойчив к NaN — неозвученные кадры
        # маскируются и НЕ размазываются на соседние озвученные. Сигнатура
        # (tensor[1, T], win_length) — по официальному README torchcrepe.
        # Выполняем ДО .double(), пока тензор float32: на MPS float64 в
        # filter не поддерживается (фикс есть только для float32).
