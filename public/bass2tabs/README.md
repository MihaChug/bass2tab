...
- питч-трекинг **CREPE-full** (torchcrepe) на MPS, чанки по 30 c, инференс в
  ...
  (не argmax): сглаживает траекторию частоты, минимизирует «октавные ошибки»;
- **обнуление уверенности в тишине** (torchcrepe.threshold.Silence, −60 дБ
  A-взвешенно + numpy-страховка по RMS) — нет фантомных нот в паузах;
